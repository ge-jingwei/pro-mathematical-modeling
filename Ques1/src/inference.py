...
def choose_device():
    ...
    index=max(rows,key=lambda r:r[1])[0]
    assert torch.cuda.is_available(),"CUDA is required by the project rules."
    torch.cuda.set_device(index); torch.set_num_threads(2)
    return torch.device(f"cuda:{index}"),dict(selected_gpu=index,gpu_table=rows,name=torch.cuda.get_device_name(index),torch_version=torch.__version__,cuda=torch.version.cuda)
