...
from dataio import load_record,transitions
from preprocess import TIME,BASE,P300
from classification import gpu_predictions
from inference import choose_device


def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--data",required=True); parser.add_argument("--out",required=True)
    args=parser.parse_args(); out=Path(args.out); checks={}
    assert np.array_equal(transitions(np.array([0,-1,-1,0,1,1,0,-1])),[1,4,7])
    checks["event_onsets_not_nonzero_samples"]=True
    path=next(Path(args.data).glob("VisualCogA_Task-1.mat")); before=load_record(path)
    ...
    device,_=choose_device(); rng=np.random.default_rng(7)
