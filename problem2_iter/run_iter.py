"""Nested source-space decoding, remote dispatch, and complete fold export."""
...
def main():
    ...
    parser.add_argument("--cache", type=Path, default=ROOT / "Ques2" / "outputs" / "final")
    ...
    import numpy as np
    import pandas as pd
    from source_features import HELPER, MODEL, ALPHAS, fit_state, feature_sets, feature_names
    from classifier import CLASSIFIERS, SIZES, fit_transformer, transform, estimator, fit_classifier, evaluate_classifier, metrics
    from utils import load_data, outer_splits, purge, save_state, mechanism_rows, finalize, dump
    np.random.seed(20260924)
    started = time.time()
    out = args.out or ROOT / "problem2_iter" / "outputs" / datetime.datetime.now().strftime("run_%Y%m%d_%H%M%S")
    if out.exists():
        raise FileExistsError(f"Refusing to overwrite: {out}")
    for folder in ["figures", "tables", "model"]:
        (out/folder).mkdir(parents=True, exist_ok=True)
    device, hardware = HELPER.choose_device()
    HELPER.setup(args.font)
    ...
    dump(out/"model/protocol.json", protocol)
    try:
        X, meta = load_data(args.cache, args.data, out)
        ...
        for outer_number, split in enumerate(splits, 1):
            ...
            for block in blocks:
                ...
                save_state(out/"model"/f"{name}.json", state)
            ...
            candidate_summary.to_csv(out/"tables"/f"{tag}_inner_search.csv", index=False)
            winners = {}
            for scheme in ["erp", "source", "combined"]:
            ...
            kept = state["training_indices"]
            save_state(out/"model"/f"{tag}_final.json", state)
            ...
            for scheme in ["erp", "source", "combined"]:
                ...
                with (out/"model"/f"{tag}_{scheme}.pkl").open("wb") as stream:
                    pickle.dump(artifact, stream)
                np.savez_compressed(out/"model"/f"{tag}_{scheme}_features.npz", train=ft[scheme], test=fe[scheme], train_indices=kept, test_indices=test)
                if scheme == "source":
            ...
            pd.DataFrame(metric_rows).to_csv(out/"tables/fold_metrics.csv", index=False)
            pd.DataFrame(predictions).to_csv(out/"tables/predictions.csv", index=False)
            dump(out/"model/split_manifest.json", manifests)
        pd.DataFrame(inner_rows).to_csv(out/"tables/inner_fold_scores.csv", index=False)
        pd.DataFrame(mechanisms).to_csv(out/"tables/separability.csv", index=False)
        dump(out/"model/feature_selection.json", selections)
        finalize(out, X, meta, device)
        dump(out/"model/completion.json", dict(status="complete", outer_folds=len(splits), unique_fits=len(state_cache), elapsed_seconds=time.time()-started))
        print(f"Completed: {out}; elapsed={time.time()-started:.1f}s", flush=True)
    except Exception as exc:
        (out/"blocked.md").write_text(f"# 运行暂停\n\n{type(exc).__name__}: {exc}\n\n不补造未运行结果。", encoding="utf-8")
        raise
...
def dispatch(args):
    ...
    import shlex
    stamp = datetime.datetime.now().strftime("q2_iter_%Y%m%d_%H%M%S")
    base = remote.ROOT_PATH.rstrip("/")+"/"+stamp
    local = args.out or ROOT/"problem2_iter/outputs"/stamp
    if local.exists():
        raise FileExistsError(f"Refusing to overwrite: {local}")
    for folder in ["problem2_iter", "Ques2"]:
    ...
    for rel in ["model/validation_epochs.npz", "model/data_audit.json", "model/mechanism_diagnostics.json", "tables/trials.csv"]:
        remote.file_upload(args.cache/rel, base+"/cache/"+str(Path(rel).parent).replace("\\", "/"))
    remote.file_upload(args.data, base, "dataset")
    font = Path("C:/Windows/Fonts/msyh.ttc")
    if font.exists():
        remote.file_upload(font, base+"/assets")
        args.font = base+"/assets/msyh.ttc"
    command = f"/home/gejw/.conda/envs/kenny_ok/bin/python -B problem2_iter/run_iter.py --server --cache cache --data dataset --out outputs --font {shlex.quote(args.font)}"
    code = remote.execute(command, workdir=base, conda_env="")
    local.mkdir(parents=True)
    remote.download(base+"/outputs", str(local))
    if code:
        raise RuntimeError(f"Remote execution failed; inspect {local}")
