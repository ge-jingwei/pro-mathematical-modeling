...
class Epochs:
    ...
    def subset(self, mask: np.ndarray) -> "Epochs":
        return Epochs(self.cue[mask], self.target[mask], self.cue_side[mask], self.target_side[mask], self.action_side[mask], self.reaction_time[mask], self.subject[mask], self.task[mask], self.file[mask], self.times, self.target_task[mask], self.target_file[mask], self.target_subject[mask])
...
def build(root: Path, cue_window: tuple[float, float] = (-0.2, 0.8), target_window: tuple[float, float] = (-0.2, 0.8)) -> Epochs:
    cue_values, target_values, cue_side, target_side, action_side, reaction_time = [], [], [], [], [], []
    ...
    for record in load_records(root / "C题" / "dataset"):
        ...
        reaction_time.append(events["reaction_time_s"].to_numpy()[kept_target])
    ...
    return Epochs(stack(cue_values), stack(target_values), stack(cue_side), stack(target_side), stack(action_side), stack(reaction_time), stack(subject), stack(task), stack(file), times, stack(target_task), stack(target_file), stack(target_subject))


def load(root: Path) -> Epochs:
    cache = root / "outputs" / "v2" / "epochs.npz"
    names = ("cue", "target", "cue_side", "target_side", "action_side", "reaction_time", "subject", "task", "source", "times", "target_task", "target_source", "target_subject")
    if cache.exists():
