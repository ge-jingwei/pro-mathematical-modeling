...
class Epochs:
    ...
    def subset(self, mask: np.ndarray) -> "Epochs":
        return Epochs(self.cue[mask], self.cue_broadband[mask], self.target[mask], self.target_broadband[mask], self.cue_side[mask], self.target_side[mask], self.action_side[mask], self.reaction_time[mask], self.target_reaction_time[mask], self.subject[mask], self.task[mask], self.file[mask], self.times, self.long_times, self.target_task[mask], self.target_file[mask], self.target_subject[mask])
...
def build(root: Path, cue_window: tuple[float, float] = (-0.2, 0.8), target_window: tuple[float, float] = (-0.2, 0.8), long_window: tuple[float, float] = (-0.2, 2.0)) -> Epochs:
    cue_values, cue_broadband, target_values, target_broadband = [], [], [], []
    cue_side, target_side, action_side, reaction_time, target_reaction_time = [], [], [], [], []
    ...
    for record in load_records(root / "C题" / "dataset"):
        ...
        _, long_t = _epochs(record.eeg, events["target_on"].to_numpy(), rate, long_window)
        ...
        broadband_long, _ = _epochs(broadband, events["target_on"].to_numpy(), rate, long_window)
        ...
        target_broadband.append(_baseline(broadband_long[kept_target], long_t, (-0.2, 0.0)))
        ...
        target_reaction_time.append(events["reaction_time_s"].to_numpy()[kept_target])
    ...
    return Epochs(stack(cue_values), stack(cue_broadband), stack(target_values), stack(target_broadband), stack(cue_side), stack(target_side), stack(action_side), stack(reaction_time), stack(target_reaction_time), stack(subject), stack(task), stack(file), times, long_times, stack(target_task), stack(target_file), stack(target_subject))


def load(root: Path) -> Epochs:
    cache = root / "outputs" / "v2" / "epochs.npz"
    names = ("cue", "cue_broadband", "target", "target_broadband", "cue_side", "target_side", "action_side", "reaction_time", "target_reaction_time", "subject", "task", "source", "times", "long_times", "target_task", "target_source", "target_subject")
    if cache.exists():
    ...
    np.savez_compressed(cache, cue=values.cue, cue_broadband=values.cue_broadband, target=values.target, target_broadband=values.target_broadband, cue_side=values.cue_side, target_side=values.target_side, action_side=values.action_side, reaction_time=values.reaction_time, target_reaction_time=values.target_reaction_time, subject=values.subject, task=values.task, source=values.file, times=values.times, long_times=values.long_times, target_task=values.target_task, target_source=values.target_file, target_subject=values.target_subject)
    return values
